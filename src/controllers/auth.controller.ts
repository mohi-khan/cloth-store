...
import { roleModel, userCompanyModel, userModel } from "../schemas";
import { eq } from "drizzle-orm";
import { changePassword, createUser, createUserCompany, getUsers, loginUser, updateUser } from "../services/auth.service";
...
export const createUserCompanyController = async (
  ...
  try {
    const { userId, companyId } = createUserCompanySchema.parse(req.body);
    const result = await createUserCompany(userId, companyId);
...
const createUserLocationSchema = z.object({
  userId: z.number().int().positive(),
...
export const checkToken =async(req:Request,res:Response,next:NextFunction) => {
  ...
  try {
    const decoded = jwt.verify("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOjEsInVzZXJuYW1lIjoibXl1c2VyIiwicm9sZSI6MSwiaWF0IjoxNzQ1Mzk2ODQxLCJleHAiOjE3NDU0ODMyNDF9.P0fDRyREff4fC7vusiwDMztuQnb9rLPTUwbvcO7Usk4", "mohikhan");
...
export const getUsersCompanyLocationVoucher = async (
  ...
  try {
    const userInfo = await db
      ...
      .innerJoin(
        userCompanyModel,
        eq(userModel.userId, userCompanyModel.userId)
      )
